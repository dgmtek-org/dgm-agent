...
import { applyCopilotAcpModelSelection, makeCopilotAcpRuntime } from "../acp/CopilotAcpSupport.ts";
import {
...
const ACP_IMPLEMENT_MODE_ALIASES = ["agent", "interactive", "default", "chat", "implement"];
const ACP_AUTOPILOT_MODE_ALIASES = ["autopilot"];
...
function findModeByAliases(
  ...
  for (const alias of normalizedAliases) {
    const exact = modes.find((mode) => {
      ...
      return id === alias || name === alias || id.endsWith(`#${alias}`);
    });
    if (exact) {
      return exact;
    }
...
function resolveRequestedModeId(input: {
  ...
  if (input.runtimeMode === "full-access") {
    return (
      findModeByAliases(modeState.availableModes, ACP_AUTOPILOT_MODE_ALIASES)?.id ??
